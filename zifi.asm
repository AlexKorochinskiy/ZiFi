...
//		call write_rtc
		...
		call test_sd_raw	;DIAG: standalone SD read test, remove once verified
		call test_sd_raw2	;DIAG: standalone SD FAT32 file read test, remove once verified
		call load_ini
	IF cable_zifi
		call psb_start
	ELSE
		call init_zifi
	ENDIF
		call set_Textpage	; set_download_dir leaves PAGE0 on the SD driver page, not Text_page
...
zifi_ready_msg		db 'ZiFi is ready.',0,0
...
parse_url
	...
	; detect http:// vs https:// (5th char: ':' for http, 's'/'S' for https)
		...
		ld a,(hl)
		pop hl
		or #20		; fold to lowercase
		cp "s"
		jr z,pu_https

pu_http		ld a,"T"
		ld (cmd_conn2site_type+0),a
		ld a,"C"
		ld (cmd_conn2site_type+1),a
		ld a,"P"
		ld (cmd_conn2site_type+2),a
		ld de,create_link_suffix_http
		ld (create_link1+1),de
		ld de,7
		ld (create_link1+4),de
		jr pu_scheme_done
...
pu_https	ld a,"S"
		...
		ld a,"L"
		ld (cmd_conn2site_type+2),a
		ld de,create_link_suffix_https
		ld (create_link1+1),de
		ld de,8
		ld (create_link1+4),de

pu_scheme_done	ld de,cmd_conn2site_adr
pu1		ld a,(hl)	; http://
		inc hl
		cp "/"
		jr nz,pu1
...
create_link1	ld hl,create_link_suffix_http
		ld bc,7
		ldir
		ld hl,modem_command
...
create_link_suffix_http
		db    #22,",80",13,10,0
create_link_suffix_https
		db    #22,",443",13,10,0
...
download_site_list	
		...
		db "Demos: bbb.retroscene.org",#0d,#0a, "https://bbb.retroscene.org/zxn_zifi.php?t=0",#0d,#0a, save_file,download_page,#0d,#0a, " ",#0d,#0a, " ",#0d,#0a
		db " Classic demos",#0d,#0a, "https://bbb.retroscene.org/zxn_zifi.php?t=1",#0d,#0a, save_file,download_page,#0d,#0a, " ",#0d,#0a, " ",#0d,#0a
		db " Most liked",#0d,#0a, "https://bbb.retroscene.org/zxn_zifi.php?t=2",#0d,#0a, save_file,download_page,#0d,#0a, " ",#0d,#0a, " ",#0d,#0a
		db " Most favorited",#0d,#0a, "https://bbb.retroscene.org/zxn_zifi.php?t=3",#0d,#0a, save_file,download_page,#0d,#0a, " ",#0d,#0a, " ",#0d,#0a
		db " ZX Enhanced",#0d,#0a, "https://bbb.retroscene.org/zxn_zifi.php?t=4",#0d,#0a, save_file,download_page,#0d,#0a, " ",#0d,#0a, " ",#0d,#0a
...
;cmd_conn2site_task_num	db "0,"
			db 0x22
cmd_conn2site_type	db "TCP"	; patched to "SSL" for https:// by parse_url
			db 0x22,",",0x22
...
;DIAG: standalone raw SD-card hardware test - bypasses WDFCVBI2 entirely,
;DIAG: talks to the SD controller directly via ports CONF=#77/DATA=#57.
;DIAG: Goal: verify our own init+read code can talk to the hardware at all,
;DIAG: before writing more of a replacement driver. Reads LBA 0 and checks
;DIAG: for the standard 55 AA boot-sector signature. Remove after testing.
test_sd_raw
		call set_Textpage
		;DIAG: pre-fill the buffer with a recognizable marker before touching
		;DIAG: the card at all -- if this marker survives a "successful" read,
		;DIAG: the read loop isn't actually writing to sdraw_buf
		ld hl,sdraw_buf
		ld de,sdraw_buf+1
		ld (hl),#ee
		ld bc,511
		...
		ld a,%00000001
		ld (sdraw_csl_val),a	;DIAG: interface 1 only -- interface 2 (#0B) hung
					;DIAG: the whole machine on real hardware, do not retry it

		di			;DIAG: off_int_dma alone didn't help -- try a real interrupt
					;DIAG: mask, in case int_main pages away our resident slot
					;DIAG: mid-read (not just an SPI timing issue)
		call sdraw_init
		or a
		jp nz,test_sd_raw_noc

		call sdraw_read0
		or a
		jp nz,test_sd_raw_rderr
		;DIAG: keep DI held through the prints too now -- ei was previously
		;DIAG: right here, leaving a window for int_main to run between the
		;DIAG: read and the dump; test whether that window is the culprit

		;DIAG: show the raw command handshake bytes -- R1 response after
		;DIAG: CMD18, and the token byte WTDO settled on (should be #FE)
		ld a,(sdraw_last_r1)
		ld de,sdraw_hexbuf3
		call sdraw_hexch_pair
		ld a,(sdraw_last_token)
		ld de,sdraw_hexbuf3+2
		call sdraw_hexch_pair
		xor a
		ld (sdraw_hexbuf3+4),a
		ld (sdraw_hexbuf3+5),a
		ld hl,sdraw_r1tok_msg
		ld b,1
		call zifi_echo
		ld hl,sdraw_hexbuf3
		ld b,1
		...
		;DIAG: always dump [0..15] and [508..511] before judging the signature,
		;DIAG: so a mismatch shows real vs. garbage/shifted bytes instead of guessing
		ld hl,sdraw_buf
		ld de,sdraw_hexbuf
		ld b,16
		call sdraw_hexdump
		ld hl,sdraw_head_msg
		ld b,1
		call zifi_echo
		ld hl,sdraw_hexbuf
		ld b,1
		...
		;DIAG: read LBA0 again, right now, and dump it too -- same bytes as
		;DIAG: above would mean a deterministic bug in our code, not a flaky card
		call sdraw_read0
		or a
		jr nz,test_sd_raw_rderr
		ld hl,sdraw_buf
		ld de,sdraw_hexbuf
		ld b,16
		call sdraw_hexdump
		ld hl,sdraw_head2_msg
		ld b,1
		call zifi_echo
		ld hl,sdraw_hexbuf
		ld b,1
		...
		ld a,(hl)
		cp #55
		jr nz,test_sd_raw_sigerr
		inc hl
		ld a,(hl)
		cp #aa
		jr nz,test_sd_raw_sigerr

		;DIAG: now read a totally different LBA (1000, not 0) -- does the
		;DIAG: garbage pattern shift/change, or stay pinned to the same values?
		ld hl,0
		ld de,1000
		call sdraw_readlba
		or a
		...
		ei			;DIAG: only now, after every print, re-enable interrupts
		ld hl,sdraw_ok_msg
		ld b,1
		call zifi_echo
		ret
...
test_sd_raw_rderr
		ei
		ld hl,sdraw_rderr_msg
		ld b,1
		call zifi_echo
		ret
test_sd_raw_sigerr
		ei
		ld hl,sdraw_sigerr_msg
		ld b,1
		call zifi_echo
		ret

sdraw_noc_msg		db 'SD raw test: FAIL, no card found',0,0
sdraw_rderr_msg		db 'SD raw test: FAIL, read error',0,0
sdraw_sigerr_msg	db 'SD raw test: no 55 AA signature (see above)',0,0
sdraw_ok_msg		db 'SD raw test: OK, signature valid',0,0
sdraw_head_msg		db 'sector0[0..15]=',0,0
sdraw_head2_msg		db '2nd read[0..15]=',0,0
sdraw_lba1000_msg	db 'LBA1000[0..15]=',0,0
sdraw_tail_msg		db 'sector0[508..511]=',0,0
sdraw_r1tok_msg		db 'R1,token=',0,0
sdraw_iface_msg		db 'interface CS_ON=',0,0
sdraw_hexbuf		ds 40
sdraw_hexbuf2		ds 16
sdraw_hexbuf3		ds 8
sdraw_last_r1		ds 1
sdraw_last_token	ds 1

sdraw_CONF	equ #77
sdraw_DATA	equ #57

;--- convert A into 2 hex ASCII chars at (DE) ---
sdraw_hexch_pair
		push af
		rra
		rra
...
;--- hex dump helper: HL=src, DE=dest ascii buffer, B=byte count ---
sdraw_hexdump
		ld a,(hl)
		push af
		rra
...
;--- low-level SD-over-SPI primitives, ported from source/DSDZC.ASM (Wild
;--- Commander Improved) which targets the same ZX Evolution SD controller.
sdraw_csh
		push bc
		push af
		ld bc,sdraw_CONF
...
sdraw_csl_val	db %00000001		;DIAG: interface 1 by default; test_sd_raw
					;DIAG: flips this to %00001011 (interface 2)
					;DIAG: if interface 1's signature check fails
sdraw_csl
		push bc
		push af
		ld bc,sdraw_CONF
...
sdraw_cycl	; DE = number of #FF clock bytes to send
		ld bc,sdraw_DATA
1		ld a,#ff
		out (c),a
		dec de
...
;--- send a pre-built 6-byte command frame pointed to by HL (real CRC).
;--- Matches D:\ST\ZP\PLAY60.ASM's actual shipped timing (not the blog
;--- article, which the article's own author -- who wrote that player --
;--- says has bugs): two dummy IN (C) reads before the frame ("; savelij"
;--- in the source), and a NOP between each byte sent. Without this, the
;--- handshake (R1/token) still looks valid but the payload comes back as
;--- deterministic garbage -- exactly what we were seeing.
sdraw_cmd
		call sdraw_csh
		call sdraw_csl
		ld bc,sdraw_DATA
...
sdraw_cmd00	db %01000000+0,0,0,0,0,#95
sdraw_cmd08	db %01000000+8,0,0,1,#aa,#87
sdraw_cmd16	db %01000000+16,0,0,2,0,#ff

sdraw_do_cmd0	ld hl,sdraw_cmd00
		jp sdraw_cmd
sdraw_do_cmd8	ld hl,sdraw_cmd08
		jp sdraw_cmd
sdraw_do_cmd16	ld hl,sdraw_cmd16
		jp sdraw_cmd
sdraw_do_cmd1	ld a,%01000000+1
		call sdraw_cmdo
		jp sdraw_resp
sdraw_do_cmd55	ld a,%01000000+55
		call sdraw_cmdo
		jp sdraw_resp
...
;--- card init: CMD0/CMD8/ACMD41 (SDv2), falls back to ACMD41/CMD1 (SDv1/MMC)
;    A=0 on success, A=1 on failure
sdraw_init
		call sdraw_csh
		ld de,512+10
		call sdraw_cycl
...
4		dec de
		ld a,d
		or e
		jr z,sdraw_init_fail
		call sdraw_do_cmd1
		jr nz,4b
		cp 1
		jr z,4b
		or a
		jr nz,sdraw_init_fail

sdraw_fbs	call sdraw_do_cmd16
		jr nz,sdraw_init_fail
		or a
		jr nz,sdraw_init_fail
		jr sdraw_init_ok

;--- SDv2 path: verify the 0x1AA echo pattern from CMD8, then ACMD41+HCS ---
sdraw_v2	ld de,#01aa
		or a
		sbc hl,de
		jr nz,sdraw_init_fail
...
5		dec de
		...
		or a
		jr nz,sdraw_init_fail
		; not distinguishing byte/block addressing (CMD58/CCS) here --
		; irrelevant for reading LBA 0, which is address 0 either way.
...
;--- read LBA 0 (single 512-byte sector) into sdraw_buf ---
;    A=0 on success, A=1 on failure
sdraw_read0
		ld hl,0
		ld de,0
...
;--- read an arbitrary LBA (single 512-byte sector) via CMD18+CMD12 into
;--- sdraw_buf. Matches the proven reference implementation in D:\ST\ZP's
;--- Z-Player source (SD-CARD2.TXT / PLAY60.ASM) byte-for-byte in approach:
;--- CMD18 (not CMD17!) for the read, and INI (not manual IN+LD(HL),A) for
;--- the bulk 512-byte transfer.
;    HL = LBA bits31-16, DE = LBA bits15-0
;    A=0 on success, A=1 on failure
sdraw_readlba
		call sdraw_csh
		call sdraw_csl
		ld bc,sdraw_DATA
		in (c)			;DIAG: settling reads + inter-byte NOPs, matching
		in (c)			;DIAG: PLAY60.ASM's CMzz exactly (see sdraw_cmd)
		ld a,%01000000+18
		out (c),a
		nop
		out (c),h
		...
		out (c),a
		call sdraw_resp
		ld (sdraw_last_r1),a	;DIAG: record for display, whatever the outcome
		or a
		jr nz,sdraw_readlba_fail

		call sdraw_wtdo
		ld (sdraw_last_token),a	;DIAG: record for display, whatever the outcome
		cp #fe
		jr nz,sdraw_readlba_fail

		;DIAG: manual IN+store loop, reloading BC (so B is pinned at 0) before
		;DIAG: every single byte -- ruling out B-register drift across INI's
		;DIAG: as a hardware port-decode issue, however unlikely
		ld hl,sdraw_buf
		ld e,32
1		ld b,16
2		ld c,sdraw_DATA
...
;--- stop a multi-block transfer (CMD12). Sent without a fresh CSH/CSL --
;--- the card is still selected from the CMD18 that started the transfer.
sdraw_do_cmd12
		ld bc,sdraw_DATA
		ld a,%01000000+12
		out (c),a
		...
		out (c),a
		xor a
		in a,(sdraw_DATA)	; one stuff byte before the R1 response, per spec
		jp sdraw_resp
...
;DIAG: step 2 - parse the FAT32 BPB (already validated in sdraw_buf's first
;DIAG: 16 bytes), locate the root directory, find ZIFI.INI's short-name
;DIAG: entry there, and read+print its first line. Proves we can navigate
;DIAG: the filesystem and read a real file, not just a raw sector.
test_sd_raw2
		di			;DIAG: real interrupt mask, see test_sd_raw
		call sdraw_read0
		or a
		jp nz,t2_fail
		...
		; acc += FATSz32 * numFATs
		ld hl,(sdraw_buf+36)
		ld (sdraw_fatsz_lo),hl
		ld hl,(sdraw_buf+38)
		ld (sdraw_fatsz_hi),hl
		ld a,(sdraw_buf+16)
		ld b,a
		or a
		jr z,t2_fatdone
t2_fatloop
		push bc
		ld hl,(sdraw_fatsz_lo)
		ld de,(sdraw_fatsz_hi)
		call sdraw_acc_add32
		pop bc
		djnz t2_fatloop
t2_fatdone
		; acc now = firstDataSector -- save it, needed again for the file itself
		ld hl,(sdraw_acc_lo)
		ld (sdraw_firstdata_lo),hl
		ld hl,(sdraw_acc_hi)
		ld (sdraw_firstdata_hi),hl

		; acc += (rootCluster-2) * sectorsPerCluster
		ld hl,(sdraw_buf+44)
		ld de,2
		or a
		sbc hl,de
...
t2_scan
		ld a,h
		cp high (sdraw_buf+512)
		jr nz,t2_chk
		ld a,l
		cp low (sdraw_buf+512)
		jr z,t2_notfound
t2_chk
		push hl
		ld de,sdraw_zifiini_name
		ld b,11
1		ld a,(de)
		cp (hl)
		jr nz,2f
...
t2_found
		; extract starting cluster: low word @ entry+26, high word @ entry+20
		push hl
		ld de,26
		add hl,de
		ld a,(hl)
		...
		ld h,(hl)
		ld l,a
		ld (sdraw_filecl_hi),hl
		pop hl

		; acc = firstDataSector + (fileCluster-2)*sectorsPerCluster
		ld hl,(sdraw_firstdata_lo)
		ld (sdraw_acc_lo),hl
		ld hl,(sdraw_firstdata_hi)
		ld (sdraw_acc_hi),hl

		ld hl,(sdraw_filecl_lo)
		ld de,2
		or a
		sbc hl,de
		ld b,h
...
t2_notfound
		ei
		ld hl,sdraw_notfound_msg
		ld b,1
		call zifi_echo
		ret
t2_fail
		ei
		ld hl,sdraw_fail2_msg
		ld b,1
		call zifi_echo
		ret

; adds a 32-bit value (DE=high word, HL=low word) into the 32-bit
; accumulator sdraw_acc_hi:sdraw_acc_lo
sdraw_acc_add32
		push de
		ld de,(sdraw_acc_lo)
		add hl,de
...
sdraw_found_msg		db 'ZIFI.INI found, first line:',0,0
sdraw_notfound_msg	db 'ZIFI.INI not found in root dir sector0',0,0
sdraw_fail2_msg		db 'SD raw test2: FAIL',0,0
sdraw_zifiini_name	db "ZIFI    INI"
...
sdraw_fatsz_lo		ds 2
sdraw_fatsz_hi		ds 2
sdraw_firstdata_lo	ds 2
sdraw_firstdata_hi	ds 2
sdraw_filecl_lo		ds 2
sdraw_filecl_hi		ds 2

		align 256
sdraw_buf	ds 512
;DIAG: end of standalone raw SD-card test block
...
get_gmt		ld b,(hl)
		...
		cp #2f
		jr c,1f
		ld a,b		; tens digit (first char) *10 ...
		and #0f
		add a
		ld c,a
		add a
		...
		add c
		ld c,a
		ld a,(hl)	; ... + ones digit (second char)
		and #0f
		add c
		ret
...
; переходим в папку downloads/<date>
set_download_dir
		call init_sd_card
		call SETROOT
		LD HL,DIR_download
...
; Standard ESP-AT SNTP commands (work on stock ESP-AT firmware, not just ours):
;   AT+CIPSNTPCFG=1,0,"pool.ntp.org"  - enable SNTP, UTC, one public server
;   AT+CIPSNTPTIME?                  -> +CIPSNTPTIME:Www Mmm dd hh:mm:ss yyyy
; The reply is UTC asctime() text; gmt_time (below) applies the "time:" offset
; from zifi.ini same as before. SNTP syncs in the background after CFG, so
; CIPSNTPTIME? may still report the 1970 epoch placeholder for a moment -
; sntp_outer_left bounds how long we wait for a real value before giving up.
; update_rtc_sw/gmt_time are patched by parse_ini from the "time:" line in zifi.ini.
update_rtc_sw	ld a,0		; do we need to sync the RTC at all? (patched: 0 = "none")

cmd_sntp_cfg	db 'AT+CIPSNTPCFG=1,0,"pool.ntp.org"',13,10,0
cmd_sntp_time	db "AT+CIPSNTPTIME?",13,10,0
...
sntp_have_line			; hl -> start of "Www Mmm dd hh:mm:ss yyyy" (buffer_cmp leaves it there)
		...
		ld de,20
		add hl,de
		ld de,str_epoch_year
		ld b,4
sntp_epoch_chk	ld a,(de)
		cp (hl)
		jr nz,sntp_real_time
		inc hl
		inc de
		djnz sntp_epoch_chk
		pop hl			; still the 1970 placeholder - SNTP hasn't synced yet
		ld a,(sntp_outer_left)
		dec a
		ld (sntp_outer_left),a
		jr z,sntp_no_sync
